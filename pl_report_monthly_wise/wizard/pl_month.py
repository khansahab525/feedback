...
class InsFinancialReportMonthWise(models.TransientModel):
    _name = "ins.financial.report.month.wise"
...
#                    if flag:
    ...
    def create(self, vals):
        ret = super(InsFinancialReportMonthWise, self).create(vals)
        return ret
    ...
    def write(self, vals):
        ...
        ret = super(InsFinancialReportMonthWise, self).write(vals)
        return ret
...
#        }
    ...
    def action_xlsx(self):
        ...
        # Get record and data
        record = self.env['ins.financial.report.month.wise'].browse(data.get('id', [])) or False
...
#                sheet.write(row_pos, 1, _('Balance'), format_header)
            report_month_wise = {}
            reconcile_codes = list( map(lambda row: row['report_lines'], datas))
            first_time = True
            h_col = 1
